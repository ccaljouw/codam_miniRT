...
/*   Updated: 2023/09/22 21:40:16 by cariencaljo   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	test_cylinder(t_px ray, t_object cylinder, float *hit_dist)
{
	...
	m2 = (v_dot(ray.direction, cylinder.vAxis) * hit_dist2) + v_dot(orig_to_center, cylinder.vAxis);
	if (hit_dist1 < 0)
	{
		hit_dist1 = hit_dist2;
		if (hit_dist1 < 0)
...
int	get_cylinder_surface_data(t_object cy, t_px px, t_scene scene)
{
	...
	top = v_add(hitpoint, v_mulitply(nAxis, cy.height));
	if (v_magnitude(v_subtract(hitpoint, top)) < (cy.diameter * 0.5))
		surface_normal_at_hitpoint = cy.vAxis;
